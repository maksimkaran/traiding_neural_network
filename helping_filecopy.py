...
def loss(input, y):
    ...
    correct_confidences = clipped_input[range(samples),y-1]
...
def loss_deriv(input,y):
    ...
    deriv = input - result
    return deriv
def standardise(file):

    candle_data = pd.read_csv(file)
    X = pd.DataFrame()
    X = pd.concat([candle_data,X])
    #X.drop(columns = 'num',axis = 1, inplace=True)
    Y = X
    Y  = Y['buy']
...
weights1 = np.random.uniform(low=-3, high=3, size=(20,5))
